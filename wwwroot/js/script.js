...
// ==========================================
// WASEEL TOGGLE SWITCH FUNCTIONALITY
// ==========================================
function initWaseelToggle() {
  const waseelToggle = document.getElementById("waseelToggle");
  const waseelSection = document.getElementById("waseelSection");
  
  if (!waseelToggle || !waseelSection) return;
  
  const waseelInputs = waseelSection.querySelectorAll("input");
  const waseelInputGroups = waseelSection.querySelectorAll(".input-group");
  
  function updateWaseelState(isEnabled) {
    if (isEnabled) {
      // Show section
      waseelSection.classList.remove("hidden");
      
      // Enable all inputs
      waseelInputs.forEach((input) => {
        input.disabled = false;
      });
      
      // Remove disabled class from input groups
      waseelInputGroups.forEach((group) => {
        group.classList.remove("disabled");
      });
    } else {
      // Hide section
      waseelSection.classList.add("hidden");
      ...
      // Add disabled class to input groups
      waseelInputGroups.forEach((group) => {
  ...
  // Set initial state
  updateWaseelState(waseelToggle.checked);
  
  // Handle toggle change
  waseelToggle.addEventListener("change", function () {
    updateWaseelState(this.checked);
  });
}
...
function generateConfigFile() {
  ...
  const dbUser = "bilal";
  const dbPassword = "hassan";

  // Waseel Data
  const waseelEnabled = document.getElementById("waseelToggle").checked;
  const waseelUser = waseelEnabled
    ? document.getElementById("waseelUser").value || ""
    : "";
  const waseelPassword = waseelEnabled
    ? document.getElementById("waseelPassword").value || ""
    : "";
  const providerId = waseelEnabled
  ...
  keys.forEach((key) => {
    ...
    if (key === "PharmacyConnection" || key === "WaseelConnection") {
      connString = `Data Source=${dataSource};Initial Catalog=${catalog};Persist Security Info=True;User ID=${dbUser};Password=${dbPassword};TrustServerCertificate=True;`;
    } else {
      connString = `Data Source=${dataSource};Initial Catalog=${catalog};Persist Security Info=True;User ID=${dbUser};Password=${dbPassword};TrustServerCertificate=True;MultipleActiveResultSets=True;`;
    }

    connectionStrings[key] = connString;
  });

  // Build ZatcaApi Object
  let zatcaApi = null;
  if (zatcaMode !== "None") {
    zatcaApi = {
  ...
  const configObject = {
    ...
    ApiSettings: {
      ...
      BeneficiaryEndpoint:
        "/beneficiaries/providers/{providerId}/patientKey/{patientId}/systemType/{typeCode}",
  ...
  // Add ZatcaApi if not None
  if (zatcaApi) {
    // Insert ZatcaApi after ConnectionStrings to maintain order preference (though JSON is unordered)
    // We can just add it to the object
    configObject["ZatcaApi"] = zatcaApi;
  }
  ...
  showInputSuccess("تم إنشاء ملف الإعدادات (JSON) بنجاح!");
}
